...
ZAPS: dict[tuple[int, str, bool], Zap] = {
    # -- Ethereum, StableSwap-NG ------------------------------------------
    # One zap for every NG metapool, whatever it is built on. Verified on a
    # mainnet fork against the World Liberty USD1 pool: a USDT deposit and
    # the withdrawal back out both matched their quotes exactly.
    (1, "0x4f493b7de8aac7d55f71853688b1f7c8f0243c85", True): Zap(
        "0xE07a16358aA878CBDa2D49A88E5106871E0db307", 3, True
    ),  # Strategic USD Reserves (USDC/USDT)
    ...
    # -- Ethereum, the older stable factory -------------------------------
    # Verified on a fork against MIM/3Crv: a USDC deposit landed 0.046%
    # under its quote, which is the two pools' fees stacking, and the
    # withdrawal back out was exact.
